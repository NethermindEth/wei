...
pub fn validate_evaluation_category(category: &agent::models::analysis::EvaluationCategory, name: &str) {
...
pub fn validate_analysis(analysis: &StructuredAnalysisResponse) {
    ...
    validate_evaluation_category(&analysis.technical_specifications, "Technical specifications");
    validate_evaluation_category(&analysis.language_quality, "Language quality");
}

/// Direct query function for the agent service
/// 
...
pub async fn direct_query_agent(_agent_service: &AgentService, prompt: &str) -> Result<String> {
    ...
    let api_key = std::env::var("WEI_AGENT_OPEN_ROUTER_API_KEY")
        .map_err(|_| agent::utils::error::Error::Internal("Missing OpenRouter API key".to_string()))?;
    
    let model_name = std::env::var("WEI_AGENT_AI_MODEL_NAME")
