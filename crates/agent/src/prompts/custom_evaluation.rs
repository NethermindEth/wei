...
pub fn generate_custom_evaluation_prompt(request: &CustomEvaluationRequest) -> String {
    ...
    let instructions = format!(
        "You are an expert governance analyst. Please evaluate this proposal focusing ONLY on the user's custom criteria above. \
    ...
    let criteria_instructions = format!(
        "IMPORTANT: You MUST use this exact structure with 'summary' and 'response_map' fields.\n\n\
    ...
    let important_instructions = format!(
    ...
    let examples = format!(
        "EXAMPLES OF GOOD CRITERION KEYS (based on different user criteria):\n\
